...
def config_file(state: testing.State, ctx: testing.Context[OdkCentralCharm]) -> pathlib.Path:
    ...
    container = next(c for c in state.containers if c.name == "service")
    return container.get_filesystem(ctx) / CONFIG_PATH.lstrip("/")


def rendered_config(state: testing.State, ctx: testing.Context[OdkCentralCharm]) -> dict[str, Any]:
    """Return the config.json the charm pushed into the service container."""
...
def test_migrations_run_then_the_service_starts(
    ...
    container = next(c for c in state_out.containers if c.name == "service")
